  ...
  const handleJoinBattle = async () => {
    ...
    try {
      ...
      if (nftData) {
        ...
        setDialogMessage('No whitelisted NFT found. Contact admin to whitelist your collection.');
      }
    } catch (error: any) {
      setDialogOpen(true);
...
`}</style>
      ...
      >
        ...
        {isConnected && !battleState && !isWaiting && (
          <div style={{ margin: '20px auto', textAlign: 'center' }}>
            <button
              ...
              style={{
                padding: '20px 40px',
                background: isJoining ? 'rgba(100, 100, 100, 0.5)' : 'linear-gradient(45deg, #00ff00, #00cc00)',
                ...
                animation: isJoining ? 'none' : 'pulseGlow 2s infinite alternate',
              }}
              onMouseEnter={(e) => {
                if (!isJoining) {
                  e.currentTarget.style.transform = 'scale(1.05)';
                  e.currentTarget.style.boxShadow = '0 0 40px rgba(0, 255, 0, 1)';
                }
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.transform = 'scale(1)';
            ...
            >
              {isJoining ? 'Joining...' : 'Join Battle Queue (3 SUI)'}
            </button>
            <p style={{
              marginTop: '15px',
        ...
        {/* Battle Info */}
        <p style={{ margin: '15px 0', fontSize: 'clamp(14px, 3.5vw, 20px)', color: '#00ffcc', padding: '0 15px' }} data-testid="text-entry-fee">
          3 SUI per Battle
        </p>
        <p style={{ marginTop: '15px', fontSize: 'clamp(14px, 3.5vw, 20px)', color: '#ffff00', padding: '0 15px' }} data-testid="text-battle-status">
          {battleStatus}
        </p>
        ...
        >
          ...
          <p style={{ color: '#00ffcc', fontSize: 'clamp(12px, 3vw, 18px)', lineHeight: '1.5', marginBottom: '15px' }}>
          ...
          <ul style={{ listStyleType: 'disc', marginLeft: '20px', color: '#00ffcc', fontSize: 'clamp(11px, 2.8vw, 16px)' }}>
