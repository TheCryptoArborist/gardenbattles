...
export default function ForestPower({ address }: ForestPowerProps) {
  ...
  const sourceRows = [
    ...
    { id: "moonbags-staking", label: "Staked" },
  ] as const;

  return (
    <div
      className={`gb-holder-rank-badge gb-hud-rank-card gb-tree-status-card ${tier.className}`}
      title={`TREE Status Rank: tier ${tierNumber} of ${TIERS.length}, ${tier.label}. Liquid TREE: ${exactBalance} TREE. Qualifying LP and staking: ${qualifyingTotal ?? "checking"} TREE.`}
    >
      ...
      <span className="gb-hud-rank-content">
        ...
        <span className="gb-tree-status-qualifying">
          Qualifying LP + staking: {qualifyingTotal === null ? "Checking…" : `${qualifyingTotal} TREE`}
        </span>
        {nextTier && (
        ...
        <span className="gb-tree-status-sources" aria-label="TREE liquidity and staking breakdown">
