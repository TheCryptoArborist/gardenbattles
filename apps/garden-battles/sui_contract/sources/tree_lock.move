module battle_garden::tree_lock {
    ...
    const LOCK_PERIOD_MS: u64 = 2_592_000_000; // 30 days
    const MAX_U64: u64 = 18_446_744_073_709_551_615;

    /// A non-transferable, wallet-owned custody receipt. `store` is deliberately
    /// omitted so third-party transfer APIs cannot move the receipt.
    public struct TreeLock<phantom T> has key {
        id: UID,
        owner: address,
        funds: Balance<T>,
        locked_at_ms: u64,
    ...
    public struct TreeLocked has copy, drop {
        lock_id: address,
        owner: address,
        amount_raw: u64,
        locked_at_ms: u64,
    ...
    public struct TreeUnlocked has copy, drop {
        lock_id: address,
        owner: address,
        amount_raw: u64,
    }
    ...
    public entry fun lock<T>(payment: Coin<T>, clock: &Clock, ctx: &mut TxContext) {
        ...
        let receipt = TreeLock<T> {
            id: object::new(ctx),
            owner,
            funds: coin::into_balance(payment),
            locked_at_ms,
            unlock_at_ms: locked_at_ms + LOCK_PERIOD_MS,
        };
    ...
    /// The owner may reclaim every locked token after maturity. There is no
    /// admin withdrawal path and no fee, yield or treasury transfer.
